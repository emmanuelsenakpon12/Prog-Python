...
import {
  MapPin, Calendar, Users, ChevronDown, Plus, Minus,
  Search, Loader2, Star,
} from "lucide-react";
...
export default function ActivitePage() {
  ...
  useEffect(() => {
    const handle = (e: MouseEvent) => {
      ...
      if (participantsRef.current && !participantsRef.current.contains(e.target as Node)) setShowParticipants(false);
    };
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (res.ok && Array.isArray(data)) setResults(data.filter((o: any) => o.type === "activite" || o.type === "activites"));
    } catch { toast.error("Erreur de chargement des activités."); }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.ok && Array.isArray(data)) {
        let filtered = data.filter((o: any) => o.type === "activite" || o.type === "activites");
        if (lieu.trim()) {
        ...
        setResults(filtered);
        if (filtered.length === 0) toast.info("Aucune activité disponible pour ces critères.");
      }
    } catch { toast.error("Erreur lors de la recherche."); }
    finally { setIsLoading(false); }
  };

  const closeAll = () => { setShowCalendar(false); setShowParticipants(false); setShowSuggestions(false); };

  const totalParticipants = adults + children;

  const displayedResults = categorie === "all"
    ? results
    : results.filter((o) => o.category === categorie);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-[#2563eb]">
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-12 lg:px-8 lg:py-16 text-center" style={{ overflow: "visible" }}>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 mb-4">
              <FerrisWheelIcon />
              <span className="text-xs font-bold text-white uppercase tracking-widest">Activités</span>
            </div>
            <h1 className="text-3xl font-extrabold text-white sm:text-4xl lg:text-5xl text-balance leading-tight">
              Vivez des expériences{" "}
              <span className="text-yellow-300">inoubliables</span>
            </h1>
            <p className="mx-auto mt-3 max-w-xl text-sm text-white/80 leading-relaxed">
              Culture, aventure, gastronomie — découvrez les meilleures activités au Bénin.
            </p>

            {/* Barre de recherche */}
            <div className="mt-8 mx-auto max-w-4xl" style={{ overflow: "visible" }}>
              <div
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col w-full min-w-0">
                    ...
                    <input
                      ...
                      placeholder="Ville ou lieu d'activité"
                      autoComplete="off"
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className={`text-sm font-semibold ${date ? "text-gray-800" : "text-gray-400"}`}>
                      {date ? format(date, "EEE. dd MMM", { locale: fr }) : "Choisir une date"}
                    </span>
                  </div>
                  {showCalendar && (
                    <div
                ...
                <div
                  ...
                  ref={participantsRef}
                  style={{ overflow: "visible" }}
                  onClick={() => { closeAll(); setShowParticipants(v => !v); }}
                >
                  <Users className="h-5 w-5 shrink-0 text-[#2563eb]" />
                  <div className="flex flex-col w-full">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Participants</span>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-gray-800">
                        {totalParticipants} personne{totalParticipants > 1 ? "s" : ""}
                      </span>
                      <ChevronDown className={`h-4 w-4 text-gray-400 transition-transform shrink-0 ml-1 ${showParticipants ? "rotate-180" : ""}`} />
                    </div>
                  </div>
                  {showParticipants && (
                    <div
                      className="absolute right-0 top-[calc(100%+8px)] z-[9999] w-64 rounded-2xl border border-gray-200 bg-white p-4 shadow-2xl space-y-4"
                      onClick={(e) => e.stopPropagation()}
                    >
                      {[
                        { label: "Adultes", desc: "18 ans et plus", value: adults, set: setAdults, min: 1 },
                        { label: "Enfants", desc: "Moins de 18 ans", value: children, set: setChildren, min: 0 },
        ...
        {/* ── Filtres catégorie ── */}
        <div className="border-b border-border bg-card/95 backdrop-blur-sm sticky top-[120px] z-30">
          <div className="mx-auto max-w-7xl px-4 lg:px-8 py-3 flex items-center gap-2 overflow-x-auto scrollbar-hide">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.id}
        ...
        <section className="mx-auto max-w-7xl px-4 lg:px-8 py-10">
          <div className="mb-6">
            {hasSearched ? (
              <h2 className="text-xl font-bold text-foreground">
                {displayedResults.length} activité{displayedResults.length !== 1 ? "s" : ""} disponible{displayedResults.length !== 1 ? "s" : ""}
                {lieu ? ` à ${lieu}` : ""}
              </h2>
            ) : (
              <h2 className="text-xl font-bold text-foreground">
                Toutes nos activités
                <span className="ml-2 text-base font-normal text-muted-foreground">({displayedResults.length})</span>
              </h2>
            )}
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="mt-5 text-lg font-bold">Aucune activité trouvée</h3>
              <p className="mt-2 text-sm text-muted-foreground max-w-xs">Modifiez vos critères ou explorez toutes nos activités.</p>
              <button onClick={() => { setCategorie("all"); fetchAll(); }} className="mt-6 rounded-xl bg-[#2563eb] px-6 py-2.5 text-sm font-bold text-white hover:bg-[#1d4ed8] transition-colors">
                Voir toutes les activités
              </button>
            </div>
          ) : (
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {displayedResults.map((o) => (
                <a
                  key={o.id}
                  href={`/offers?type=activite`}
                  ...
                  <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                    {o.images && o.images.length > 0 ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <span className="text-muted-foreground/20"><FerrisWheelIcon /></span>
                      </div>
                    )}
