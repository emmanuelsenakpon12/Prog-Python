...
# Overlays A* bidirectionnel (RGBA)
COL_VISITE     = (110, 220, 255, 150)   # closed fwd (bleu)
COL_VISITE_BWD = (255, 160, 80,  150)   # closed bwd (orange)
COL_A_EXPLORER = (255, 220, 120, 160)   # open fwd (jaune)
COL_A_EXPLORER_BWD = (255, 120, 60, 160) # open bwd (orange vif)
COL_COURANT    = (120, 175, 255, 190)   # courant fwd
COL_COURANT_BWD= (255, 100, 60,  190)   # courant bwd
COL_JONCTION   = (255, 255, 80,  220)   # point de jonction
...
# ============================================================
# 4) A* BIDIRECTIONNEL (LOGIQUE ISOLÉE)
# ============================================================
# Principe :
# - Deux A* simultanés : AVANT (S→E) et ARRIÈRE (E→S).
# - On alterne une étape fwd / une étape bwd à chaque tour.
# - Jonction : un nœud extrait est déjà dans le closed set de l'autre sens.
# - Chemin = chemin_fwd (S→jonction) + chemin_bwd (jonction→E).
...
def bidir_initialiser(depart, arrivee):
    def _init(src, dst):
        pq = []
        h0 = heuristique_manhattan(src, dst)
        heapq.heappush(pq, (h0, 0, src))
        return {
            "pq": pq,
            "visite": set(),
            "frontiere": {src},
    ...
    return {
        "fwd": _init(depart, arrivee),
        "bwd": _init(arrivee, depart),
        "jonction": None,
        "meilleur": float("inf"),
        "termine": False,
        "trouve": False,
...
def _bidir_etape_sens(grille, etat, sens, cible, couts):
    s = etat[sens]
    autre = "bwd" if sens == "fwd" else "fwd"
    pq, visite, frontiere, parent, g = (
        s["pq"], s["visite"], s["frontiere"], s["parent"], s["g"]
    )
    while pq:
    ...
    visite.add(courant)
    s["courant"] = courant
    if courant in etat[autre]["visite"]:
        g_total = g_cur + etat[autre]["g"].get(courant, float("inf"))
        if g_total < etat["meilleur"]:
            etat["meilleur"] = g_total
            etat["jonction"] = courant
        etat["termine"] = True
        etat["trouve"] = True
        return courant
    for rr, cc, _ in voisins_4(grille, *courant):
        ...
        if new_g < g.get(nxt, float("inf")):
            g[nxt] = new_g
            parent[nxt] = courant
            h = heuristique_manhattan(nxt, cible)
            heapq.heappush(pq, (new_g + h, new_g, nxt))
            frontiere.add(nxt)
    return courant

def bidir_faire_une_etape(grille, etat, couts):
    if etat["termine"]:
        return
    if etat["tour"] == "fwd":
        res = _bidir_etape_sens(grille, etat, "fwd", etat["arrivee"], couts)
        etat["tour"] = "bwd"
    else:
        res = _bidir_etape_sens(grille, etat, "bwd", etat["depart"], couts)
        etat["tour"] = "fwd"
    if res is None:
        etat["termine"] = True
        etat["trouve"] = False

def bidir_reconstruire_chemin(etat):
    jonction = etat["jonction"]
    if jonction is None:
        return None
    parent_fwd = etat["fwd"]["parent"]
    parent_bwd = etat["bwd"]["parent"]
    chemin_fwd = []
    cur = jonction
    while cur is not None:
        chemin_fwd.append(cur)
        cur = parent_fwd.get(cur)
    chemin_fwd.reverse()
    chemin_bwd = []
    cur = parent_bwd.get(jonction)
    while cur is not None:
        chemin_bwd.append(cur)
        cur = parent_bwd.get(cur)
    return chemin_fwd + chemin_bwd
...
class AppliAStar:
    ...
    def __init__(self, grille):
        ...
        """
        pygame.init()
        pygame.display.set_caption("Labyrinthe A* Bidirectionnel (fwd=bleu | bwd=orange)")
    ...
    def reinitialiser_tout(self):
        ...
        self.g = {}
        self.visite_bwd = set()
        self.frontiere_bwd = set()
        self.courant_bwd = None
        self.jonction = None
    ...
    def _calculer_solution_astar(self):
        ...
        Returns:
            None
        """
        etat = bidir_initialiser(self.depart, self.sortie)
        while not etat["termine"]:
            bidir_faire_une_etape(self.grille, etat, self.couts)

        if etat["trouve"]:
            self.etat_solution = etat
            self.chemin_opt = bidir_reconstruire_chemin(etat)
            # coût = somme des coûts d'entrée du chemin
            if self.chemin_opt:
                self.cout_opt = sum(cout_case(self.couts, p) for p in self.chemin_opt[1:])
            else:
                self.cout_opt = None
        else:
            self.etat_solution = None
            self.chemin_opt = None
            self.cout_opt = None
    ...
    def _sync_depuis_etat_algo(self):
        ...
        e = self.etat_algo
        # Fwd
        self.visite     = set(e["fwd"]["visite"])
        self.frontiere  = set(e["fwd"]["frontiere"])
        self.courant    = e["fwd"]["courant"]
        self.parent     = dict(e["fwd"]["parent"])
        self.g          = dict(e["fwd"]["g"])
        # Bwd
        self.visite_bwd    = set(e["bwd"]["visite"])
        self.frontiere_bwd = set(e["bwd"]["frontiere"])
        self.courant_bwd   = e["bwd"]["courant"]
        self.jonction      = e.get("jonction")

        self.vu = self.visite | self.frontiere | self.visite_bwd | self.frontiere_bwd
        if self.courant:     self.vu.add(self.courant)
        if self.courant_bwd: self.vu.add(self.courant_bwd)
        ...
        if e.get("termine") and e.get("trouve"):
    ...
    def dessiner_barre_bas(self):
        ...
        jonc = str(self.jonction) if self.jonction else "—"
        self._dessiner_texte(12, y + 8, f"Coût optimal : {opt}  |  Jonction : {jonc}", self.font_petit)
        ...
        for r in range(self.lignes):
            for c in range(self.colonnes):
                ...
                if ch != "#":
                    ...
                    # Bwd (orange)
                    if pos in self.visite_bwd:
                        dessiner_overlay_rgba(self.ecran, rect, COL_VISITE_BWD, radius=7, outline=(255, 200, 100, 120))
                    if pos in self.frontiere_bwd:
                        dessiner_overlay_rgba(self.ecran, rect, COL_A_EXPLORER_BWD, radius=7)
                    if self.courant_bwd == pos:
                        dessiner_overlay_rgba(self.ecran, rect, COL_COURANT_BWD, radius=7, outline=(255, 200, 100, 170))
                    # Jonction (étoile jaune vif)
                    if self.jonction == pos:
                        dessiner_overlay_rgba(self.ecran, rect, COL_JONCTION, radius=7, outline=(255, 255, 200, 255))
            ...
            for ev in pygame.event.get():
                ...
                if ev.type == pygame.KEYDOWN:
                    ...
                    if ev.key == pygame.K_e:
                        ...
                        self.mode = "auto"
                        self.dernier_event_auto = 0
                        self.etat_algo = bidir_initialiser(self.depart, self.sortie)
                        self._sync_depuis_etat_algo()

                    # ESPACE = A* bidirectionnel pas à pas
                    if ev.key == pygame.K_SPACE:
                        ...
                        self.mode = "step"
                        bidir_faire_une_etape(self.grille, self.etat_algo, self.couts)
                        self._sync_depuis_etat_algo()
